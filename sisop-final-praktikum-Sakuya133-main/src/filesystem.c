...
void fsRead(struct file_metadata* metadata, enum fs_return* status) {
  ...
  metadata->filesize = 0;
  sector_index = node.data_index;

  for (i = 0; i < FS_MAX_SECTOR; i++) {
    byte sector_number = data_fs_buf.datas[sector_index].sectors[i];
...
void fsWrite(struct file_metadata* metadata, enum fs_return* status) {
  ...
  needed_blocks = (metadata->filesize + SECTOR_SIZE - 1) / SECTOR_SIZE;
  ...
  if (free_block_count < needed_blocks) {
  ...
  for (i = 0; i < SECTOR_SIZE && j < needed_blocks; i++) {
    if (!map_fs_buf.is_used[i]) {
      ...
      data_fs_buf.datas[free_data_index].sectors[j] = (byte)i;
